    ...
    public function updateContest()
    {
        try {
            ...
            foreach ($recentViolationsToday as $violation) {
                ...
                // Update completeness symbols for each TasFile
                $violation->checkCompleteness();
            }
      
            // Pass data to the view, including the new variable $violationData
            return view('tas.edit', compact('recentViolationsToday', 'violations', 'officers'));
        } catch (\Exception $e) {
...
public function analyticsDash()
{
    ...
    // Calculate forecast
    $forecastData = $this->calculateForecast($data);
...
{
    // Simple forecasting algorithm (just average of the last 3 data points)
    $lastThreeValues = array_slice($data, -3); // Get the last three elements of the array

    $forecast = array_sum($lastThreeValues) / count($lastThreeValues);
...
public function generateAdditionalMessages()
{
    ...
    // Calculate the average number of violations per day
    $totalViolations = $recentViolations->count();
    $firstViolationDate = Carbon::parse($recentViolations->last()->created_at);
    $lastViolationDate = Carbon::parse($recentViolations->first()->created_at);
    $daysDiff = $firstViolationDate->diffInDays($lastViolationDate) ?: 1; // Ensure at least 1 day to avoid division by zero
    $averageViolationsPerDay = $totalViolations / $daysDiff;

    // Analyze the trend of violations
    $trendMessage = '';
    if ($averageViolationsPerDay > 0) {
        $trendMessage = 'The average number of violations per day is ' . number_format($averageViolationsPerDay, 2) . '.';
    }

    // Check if the violation count is increasing or decreasing
    $currentCount = $recentViolations->first()->count();
    $previousCount = $recentViolations->last()->count();
    ...
    $additionalMessages[] = 'Violation count trend: ' . ucfirst($trend) . '.';
    $additionalMessages[] = $trendMessage;

    // Predict future violations based on trend
    $predictionMessage = '';
    if ($trend === 'increasing') {
        $predictionMessage = 'Based on the current trend, we anticipate more violations in the coming days.';
    } elseif ($trend === 'decreasing') {
        $predictionMessage = 'Based on the current trend, we anticipate a decrease in violations in the coming days.';
    } else {
        $predictionMessage = 'Based on the current trend, we anticipate a stable number of violations in the coming days.';
    }
...
$peakTime = $violationTimes->countBy(function ($violationTime) {
    ...
    if ($peakTime) {
        $additionalMessages[] = 'Peak violation time: ' . $peakTime;
    }

    if ($violationLocations) {
        $additionalMessages[] = 'Peak violation location: ' . $violationLocations;
    }

    
 

    // Calculate the average number of violations per day for recent data
    $totalRecentViolations = $recentViolations->count();
    $firstViolationDate = Carbon::parse($recentViolations->last()->created_at);
    $lastViolationDate = Carbon::parse($recentViolations->first()->created_at);
    $daysDiff = $firstViolationDate->diffInDays($lastViolationDate) ?: 1; // Ensure at least 1 day to avoid division by zero
    $averageRecentViolationsPerDay = $totalRecentViolations / $daysDiff;
    ...
    $daysDiffHistorical = $historicalStartDate->diffInDays($historicalEndDate) ?: 1; // Ensure at least 1 day to avoid division by zero
    $averageHistoricalViolationsPerDay = $totalHistoricalViolations / $daysDiffHistorical;

    // Compare recent data with historical data
    $comparisonMessage = '';
    if ($averageRecentViolationsPerDay > $averageHistoricalViolationsPerDay) {
        $comparisonMessage = 'The current violation rate is higher than the historical average.';
    } elseif ($averageRecentViolationsPerDay < $averageHistoricalViolationsPerDay) {
        $comparisonMessage = 'The current violation rate is lower than the historical average.';
    } else {
        $comparisonMessage = 'The current violation rate is consistent with the historical average.';
    }

    $additionalMessages[] = 'Comparison with historical data: ' . $comparisonMessage;
