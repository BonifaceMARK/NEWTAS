<div class="modal fade" id="finishModal-{{ $InventoryItem->id }}" tabindex="-1" role="dialog" aria-labelledby="finishModalLabel-{{ $InventoryItem->id }}" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="{{ route('finish.case', ['id' => $InventoryItem->id]) }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title" id="finishModalLabel-{{ $InventoryItem->id }}">Finish Case</h5>
                </div>
        ...
        <div class="card border-0 shadow">
            ...
            <div class="card-body mt-3">
                <div class="mb-4">
                    ...
                    <p class="fw-bold">{{ $InventoryItem->resolution_no }}</p>
                </div>
                <hr>
                <div class="mb-4">
                    <h6 class="text-muted">Driver:</h6>
                    <p class="fw-bold">{{ $InventoryItem->driver }}</p>
                </div>
                <hr>
                <div class="mb-4">
                    <h6 class="text-muted">Contact No:</h6>
                    <p class="fw-bold">{{ $InventoryItem->contact_no }}</p>
                </div>
                <hr>
                <div class="mb-4">
                    <h6 class="text-muted">TOP:</h6>
                    <p class="fw-bold">{{ $InventoryItem->top ?? 'N/A' }}</p>
                </div>
                <hr>
                <div class="mb-4">
                    <h6 class="text-muted">Transaction No:</h6>
                    <p class="fw-bold">{{ $InventoryItem->transaction_no ?? 'N/A' }}</p>
                </div>
                <hr>
                <div class="mb-4">
                    <h6 class="text-muted">Received Date:</h6>
                    <p class="fw-bold">{{ $InventoryItem->date_received }}</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card border-0 shadow">
            ...
            <div class="card-body mt-3">
                <div class="mb-4">
                    ...
                    <p class="fw-bold">{{ $InventoryItem->plate_no }}</p>
                </div>
                <hr>
                <div class="mb-4">
                    <h6 class="text-muted">Apprehending Officer:</h6>
                    <p class="fw-bold">{{ $InventoryItem->apprehending_officer ?? 'N/A' }}</p>
                </div>
                <hr>
                <div class="mb-4">
                    <h6 class="text-muted">Date Recorded:</h6>
                    <p class="fw-bold">{{ $InventoryItem->created_at }}</p>
                </div>
                <hr>
                <div class="mb-4">
...
<div class="row mb-4">
        ...
        <div class="card border-0 shadow">
            ...
            <div class="card-body mt-3">
                ...
                <form action="{{ route('admitremark') }}" id="remarksForm" method="POST" class="remarksForm">
                    @csrf
                    <input type="hidden" name="tas_file_id" value="{{ $InventoryItem->id }}">
                    <div class="mt-3">
    ...
    <div class="col-md-6">
        <div class="card border-0 shadow">
            ...
            <div class="card-body mt-3">
                @if (!is_null($InventoryItem->file_attach))
                    @php
                        $decodedFiles = json_decode($InventoryItem->file_attach, true);
                    @endphp
...
<div class="modal-footer">
    <a href="{{ route('print.sub', ['id' => $InventoryItem->id]) }}" class="btn btn-primary" onclick="openInNewTabAndPrint('{{ route('print.sub', ['id' => $InventoryItem->id]) }}'); return false;">
        <span class="bi bi-printer"></span> Print Subpeona
    </a>
    <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#finishModal{{ $InventoryItem->id }}">Finish</button>
    <form action="{{ route('update.status', ['id' => $InventoryItem->id]) }}" method="POST" style="display:inline;">
        @csrf
